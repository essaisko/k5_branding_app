...
class CompactLogoButton extends ConsumerWidget {
  ...
  void _showLogoSelectionDialog(
    ...
    try {
      ...
      if (uniqueLogoPaths.isEmpty) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('선택 가능한 로고가 없습니다.')),
          );
        }
        return;
      }
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: GridView.builder(
              ...
              itemCount: uniqueLogoPaths.length,
              itemBuilder: (context, index) {
                final logoPath = uniqueLogoPaths[index];
                return InkWell(
                  onTap: () {
                    try {
  ...
  Widget _buildSafeImage(String path) {
    return Image.asset(
      ...
      errorBuilder: (context, error, stackTrace) {
        dev.log('이미지 로드 오류: $path - $error', name: 'CompactLogoButton');
